...
const CharacterOverview = () => {
  ...
  const calculateExpPercentage = (current: number, nextLevel: number): string => {
    if (nextLevel === 0) return '0%';
    const percentage = (current / nextLevel) * 100;
    return `${percentage}%`;
  };

  const expPercentage = useMemo(() => calculateExpPercentage(userCharacter.experience, 20), [userCharacter.experience]);
      ...
      <View className="w-[35%] h-[95%] relative mt-auto mb-auto">
        ...
        >
          ...
          <View className="bg-red-50 ml-1 mt-[0.15rem] min-w-[76%] max-h-[70%] rounded-full overflow-hidden">
            <View style={{width: expPercentage}} className={`bg-green-400 h-full rounded-l-full rounded-r-lg flex items-center justify-center`}>
            </View>
          </View>
        </View>
      </View>
